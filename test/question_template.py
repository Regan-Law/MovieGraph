...
class QuestionTemplate:
    ...
    def get_question_answer(self, question, template):  # 获取问题的答案
        # 如果问题模板的格式不正确则结束
    ...
    def get_movie_introduction(self):
        ...
        print(cql)
        answer = self.graph.run(cql)[0]
        final_answer = movie_name + "简介:" + str(answer)
        return final_answer

    # 4:nm 演员列表
    def get_movie_actor_list(self):
        movie_name = self.get_movie_name()
        cql = f"match(n:Person)-[r:actedin]->(m:Movie) where m.title contains '{movie_name}' return n.name"
        print(cql)
        answer = self.graph.run(cql)
        answer_set = set(answer)
        answer_list = list(answer_set)
    ...
    def get_actor_movie_type(self):
        actor_name = self.get_name("nr")
        # 查询电影名称
        cql = f"match(n:Person)-[]->(m:Movie) where n.name contains '{actor_name}' return m.title"
        print(cql)
